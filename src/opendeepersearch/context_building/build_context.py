...
def extract_top_stories(top_stories: Optional[List[Dict]]) -> List[str]:
    ...
    return [item["title"] for item in top_stories if "title" in item]


def extract_answer_box(answer_box: Optional[Dict]) -> List[str]:
...
def build_context(
    ...
    try:
        ...
        answer_box = extract_answer_box(sources_result.get("answerBox"))
